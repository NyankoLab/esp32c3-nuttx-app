 ...
 *
 * \warning If building without MBEDTLS_MD_C, you must call esp_psa_crypto_init()
 ...
 * esp_psa_crypto_init() before doing any TLS operations.
 *
 ...
 * \c esp_mbedtls_ssl_tls13_conf_max_early_data_size() for more information.
 *
 ...
 * esp_mbedtls_platform_zeroize(). This replaces the default implementation in
 * platform_util.c.
 *
 * esp_mbedtls_platform_zeroize() is a widely used function across the library to
 * zero a block of memory. The implementation is expected to be secure in the
 * sense that it has been written to prevent the compiler from removing calls
 * to esp_mbedtls_platform_zeroize() as part of redundant code elimination
 * optimizations. However, it is difficult to guarantee that calls to
 * esp_mbedtls_platform_zeroize() will not be optimized by the compiler as older
 * versions of the C language standards do not provide a secure implementation
 * of memset(). Therefore, MBEDTLS_PLATFORM_ZEROIZE_ALT enables users to
 * configure their own implementation of esp_mbedtls_platform_zeroize(), for
 * example by using directives specific to their compiler, esp_features from newer
